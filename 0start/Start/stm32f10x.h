...
typedef struct
{
  __IO uint32_t CRL;                    //0-7 pins, MODEn[4*n+1:4*n], CNF[4*n+3:4*n+2]
  /*
  CNF: INPUT MODE (MODE[1:0]==00)
        00: Analog Mode 
        01: Floating Input
        10: Input With Pull-up/Pull-down
       OUPUT MODE (MODE[1:0]>00)
        00: General purpose ouput push-pull
        01: General purpose ouput open-drain
  MODE: 00: Input mode (reset)
        01: Output mode max speed 10MHz
        10: Output mode max speed 2MHz
        11: Output mode max speed 50MHz
  */
  __IO uint32_t CRH;                    //8-15 pins,
  __IO uint32_t IDR;                    //InputDataRegister, Bits0-15: IDRn[n]
  __IO uint32_t ODR;                    //OutputDataResgister, Bits0-15: ODRn[n]
...
typedef struct
{
  __IO uint32_t CR;                   //ClockControlRegister
  /*
  bit25:  PLL clock ready flag
          Set by hardware  0: PLL unlocked  1: PLL locked
  bit24:  PLL enable
          Set and clear by software to enable PLL. Clear by hardware.
  bit19:  Clock security system enable
  bit18:  External high-speed clock bypass
          Set and clear by software to bypass the oscillator with an external clock.
          0: not bypassed
          1: bypassed with external clock
  bit17:  External high-speed clock ready flag
          Set by hardware
  bit16:  HSE clock enable
          Set and clear by software
  bit15-8:  Internal high-speed clock callibration
            These bits are initialized automatically at startup.
  bit7-3:   Internal high-speed clock trimming
  bit1:  Internal high-speed clock ready flag
         Set by hardware
  bit0:  Internal high-speed clock enable
         Set and clear by software.
         Set by hardware force the internal 8MHz oscillator ON when leaving Stop or Stannby mode or in case of failure...
  **/
  __IO uint32_t CFGR;                 //ConfigureRegister
/*
  bit26-24:   Microcontroller clock output
              Set and clear by software
              0xx: No clock
              100: SYSCLK selected
  ...
  bit21-18:   PLL multiplication factor
              These bits are written by software to define the PLL multipication factor.
              These bits can be written only when PLL is disabled.
              MULfactor = data + 2    if data == 15 : MULfactor = 16
  bit17:      HSE divider for PLL entry
              Set and clear by software to divide HSE before entering PLL.
              1: HSE clock divided by 2
  bit16:      PLL entry clock source
              Set and clear by software to select PLL clock source.
              0: HSI oscillator clock / 2 selected as PLL input clock
              1: HSE oscillator clock selected as PLL input clock
  bit15-14:   DC prescaler
  bit13-11:   APB high-speed prescaler (APB2)
              0xx: HCLK not divided
              100: HCLK divided by 2
              101: HCLK divided by 4
              110: HCLK divided by 8
              111: HCLK divided by 16
  bit10-8:    APB low-speed prescaler (APB1)
  bit7-4:     AHB prescaler
  ...
  bit3-2: System Clock switch status
          Set and clear by hardware to indicate which clock source is used as system clock
          00: HSI 
          01: HSE
          10: PLL
          11: not applicable
  bit1-0: System clock switch
          Set and clear by software to select SYSCLK source
          00: HSI
          01: HSE
          10: PLL
          11: not allowed
*/
  __IO uint32_t CIR;                    //ClockInterruptRegister
  /*
  *1: Clear Interrupt Flag
  *0: No effect

  bit23:    Clock security system interrupt clear.
  bit20:    PLL ready interrupt clear.
  bit19:    HSE ready interrupt clear.
  bit18:    HSI ready interrupt clear.
  bit17:    LSE ready interrupt clear.
  bit16:    LSI ready interrupt clear.
  bit12:    PLL ready interrupt enable
  bit11:    HSE ready interrupt enable
  bit10:    HSI ready interrupt eanable
  bit9:     LSE ready interrupt enable
  bit8:     LSI ready interrupt enable
  bit7:     CLock security interrupt flag.
            0: No Failure
            1: HSE clock failure
  bit4:     PLL ready interrupt flag.
  bit3:     HSE ready interrupt flag.
  bit2:     HSI ready interrupt flag.
  bit1:     LSE ready interrupt flag.
  bit0:     LSI ready interrupt flag.
  */
  __IO uint32_t APB2RSTR;             //APB2ResetRegister        
  /*
  
  */  
  __IO uint32_t APB1RSTR;
  __IO uint32_t AHBENR;               //AHB_EnbaleRegister
  ...
  bit15:    ADC3ClockEN
  bit14:    USART1ClockEN
  bit13:    TIM8ClockEN TIM 775554332111qwetyuiop17777767676776710002iu8888ooooo99999qwertye25fffrrrfrrrrrrrrdr44444eerfrrrrrrrrr4rrrrrrrrrrrrrrrrrrrrrrrrrrrrtyuiopp{}{}\\////..,.,.,.,.,.,.,;'
  bit12:    SPI1ClockEN
  bit11:    TIM1ClockEN
  bit10:    ADC2ClockEN
  bit9:     ADC1ClockEN
...
typedef struct
{
  ...
  __IO uint16_t DIER;
  uint16_t  RESERVED3;
  __IO uint16_t SR;                       //StateRegister
  /*
  bit12:  
  */
  uint16_t  RESERVED4;
